...
class TestFileUploadSecurity:
    ...
    def test_accept_xlsx_extension(self):
...
class TestDDAValidation:
    def test_prior_alpha_too_low(self):
        r = client.post(
            "/api/dda/run",
            json=[{"lead_id": "L1", "channels": ["meta"], "converted": True}],
            params={"prior_alpha": PRIOR_ALPHA_MIN - 0.001},
        )
        # DDA endpoint uses Body, so we need to pass as JSON body
        r = client.post(
            "/api/dda/run",
            json={
    ...
    def test_prior_alpha_too_high(self):
        r = client.post(
            "/api/dda/run",
            json={
                "journeys": [{"lead_id": "L1", "channels": ["meta"], "converted": True}],
    ...
    def test_invalid_journey_format(self):
        r = client.post(
            "/api/dda/run",
            json={
                "journeys": [{"bad_key": "x"}],
    ...
    def test_journey_stats_invalid_format(self):
        r = client.post(
...
class TestCSVEndpointSecurity:
    def test_run_from_csv_rejects_bad_extension(self):
    ...
    def test_run_from_csv_rejects_large_file(self):
    ...
    def test_run_from_csv_invalid_csv(self):
